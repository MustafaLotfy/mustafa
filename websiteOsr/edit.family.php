...
if(isset($_POST['edit-submit'])& isset($_SESSION['id'])) {
    ...
    $name = mysqli_real_escape_string(Database::conn(), $_POST['first_name']);
    ...
    if ((!preg_match("/^[a-zA-Z]*$/", $name) & !empty($first_name)) ||
        ...
        if (!Database::check_email_existance($_SESSION['email'])) {
            ...
            if (!Database::checkPassword_family($password_conformation)) {header("Location: ../profile.php?profile=mismatch");exit();}
            else{
                if (Database::check_email_existance_family($email)&!empty($email)) {header("Location: ../profile.php?profile=exists");exit();}
                else {
                    Database::update_family($name,$last_name,$email,$password,$address,$country_name,$phone_number);
